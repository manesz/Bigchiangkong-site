...
function portfolio_register() {
    $labels = array(
        'name' => __( 'Portfolio', 'cabana' ),
        'add_new' => __( 'Add New', 'cabana' ),
        'add_new_item' => __( 'Add New Portfolio Item', 'cabana' ),
        'edit_item' => __( 'Edit Portfolio Item', 'cabana' ),
        'new_item' => __( 'New Portfolio Item', 'cabana' ),
        'view_item' => __( 'View Portfolio Item', 'cabana' ),
        'search_items' => __( 'Search Portfolio Items', 'cabana' ),
...
function portfolio_help_text( $contextual_help, $screen_id, $screen ) {
    if ( 'portfolio' == $screen->id ) {
        $contextual_help =
        '<h3>' . __( 'Things to remember when adding a Portfolio item:', 'cabana' ) . '</h3>' .
        '<ul>' .
        '<li>' . __( 'Give the item a title. The title will be used as the item\'s headline.', 'cabana' ) . '</li>' .
        '<li>' . __( 'You can choose to insert either single images, image slideshow, audio, or video.', 'cabana' ) . '</li>' .
        '<li>' . __( 'Enter your portfolio item overview into the Visual or HTML area. The text will appear for your project overview.', 'cabana' ) . '</li>' .
        '<li>' . __( 'Choose (or first create) a Project Type. You will need to use these to enable the filterable Portfolio option.', 'cabana' ) . '</li>' .
        '</ul>';
    }
    elseif ( 'edit-portfolio' == $screen->id ) {
        $contextual_help = '<p>' . __( 'A list of all Portfolio items appear below. To edit an item, click on the items\'s title.', 'cabana' ) . '</p>';
    }
...
$meta_box_portfolio = array(
	...
	'title' =>  __( 'Portfolio Detail Settings', 'cabana' ),
	'page' => 'portfolio',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
		array(
			'name' => __( 'Project Icons', 'cabana' ),
		    'desc' => __( 'Add icons for your portfolio item. For example if it is a video, you could add a video camera icon and a Vimeo icon.<br /><br />A list of all available icons can be found <a href="http://fortawesome.github.io/Font-Awesome/icons/" target="_blank">here</a><br /><br />Enter icon code in the following shortcode format:<br /><strong>[icon name=fa-video-camera]</strong>', 'cabana' ),
		    'id' => $prefix . 'project_icons',
		    'type' => 'text',
		    'std' => ''
		),
    	array(
			'name' =>  __( 'Project Type', 'cabana' ),
			'desc' => __( 'Choose the type of project you wish to display.', 'cabana' ),
			'id' => $prefix . 'project_type',
			"type" => "select",
			'std' => 'Image',
			'options' => array( 'Images', 'Slideshow', 'Video', 'Audio' )
		),
		array(
    	   'name' => __( 'Client Name', 'cabana' ),
    	   'desc' => __( 'Client who the project was for', 'cabana' ),
    	   'id' => $prefix . 'client_name',
    	   'type' => 'text',
    	   'std' => ''
    	),
    	array(
    	   'name' => __( 'Project Checklist', 'cabana' ),
    	   'desc' => __( 'Create a checklist of skills or tools used on the project<br />(ie; Canon EOS 5D, Branding, Responsive Design, WordPress etc...)', 'cabana' ),
    	   'id' => $prefix . 'project_checklist',
    	   'type' => 'textarea',
    	   'std' => ''
    	),
    	array(
    	   'name' => __( 'Project Date', 'cabana' ),
    	   'desc' => __( 'Enter the date the project was completed', 'cabana' ),
    	   'id' => $prefix . 'project_date',
    	   'type' => 'text',
    	   'std' => ''
    	),
    	array(
    	   'name' => __( 'Project URL', 'cabana' ),
    	   'desc' => __( 'Enter the URL for this project', 'cabana' ),
...
$meta_box_portfolio_portfolio_image = array(
	...
	'title' => __( 'Image Settings', 'cabana' ),
	'page' => 'portfolio',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
...
$meta_box_portfolio_portfolio_video = array(
	...
	'title' => __( 'Video Settings', 'cabana' ),
	'page' => 'portfolio',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
		array(
			'name' => __( 'Video Embed Code', 'cabana' ),
			'desc' => __( 'If you are using video from somewhere like YouTube, Vimeo etc... Please paste the embed code here. Width should be at least 960px with any height.<br><br>', 'cabana' ),
...
$meta_box_portfolio_portfolio_audio = array(
	...
	'title' => __( 'Audio Settings', 'cabana' ),
	'page' => 'portfolio',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
		array(
			'name' => __( 'Audio Embed Code', 'cabana' ),
			'desc' => __( 'If you are using audio from somewhere like SoundCloud, please paste the embed code here. If you have multiple audio clips, you can paste them one under the other.', 'cabana' ),
...
function gt_add_box_portfolio() {
	global $meta_box_portfolio, $meta_box_portfolio_portfolio_image, $meta_box_portfolio_portfolio_video, $meta_box_portfolio_portfolio_audio;
...
function gt_show_box_portfolio_image() {
	...
	echo '<p style="padding:10px 0 0 0; font-weight: normal; color:#666;">'.__( 'Upload images to be used for this project item (images should be at least 960px wide).<br />Set a Featured Image (to the box on the right) that will be displayed on the homepage, and portfolio index pages. Then upload the main images to be used in your showcase using the Upload button below.', 'cabana' ).'</p>';
	// Use nonce for verification
	echo '<input type="hidden" name="gt_meta_box_nonce" value="', wp_create_nonce( basename( __FILE__) ), '" />';
	...
	foreach ( $meta_box_portfolio_portfolio_image['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
			...
			?>
				...
				jQuery(function($) {
					...
					$('#gt_images_upload').on('click', function(e) {
						...
						var options = {
							title: '<?php _e( "Create Featured Gallery", "cabana" ); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						...
						if( frame || selection ) {
							options['title'] = '<?php _e( "Edit Featured Gallery", "cabana" ); ?>';
						}
						...
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php _e( "Edit Featured Gallery", "cabana" ); ?>';
						...
						function overrideGalleryInsert() {
    						frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php _e( "Save Featured Gallery", "cabana" ); ?>',
									...
									click: function() {
										...
										this.el.innerHTML = '<?php _e( "Saving...", "cabana" ); ?>';
			...
			<?php
				...
				$thumbs_output = '';
				$button_text = ($meta) ? __( 'Edit Gallery', 'cabana' ) : $field['std'];
				if( $meta ) {
					$field['std'] = __( 'Edit Gallery', 'cabana' );
					$thumbs = explode(',', $meta);
					$thumbs_output = '';
